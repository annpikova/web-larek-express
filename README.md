...
1. **XSS (Межсайтовый скриптинг)**
   ...
   - **Запрещена загрузка SVG файлов** (фронт и бэкенд) для предотвращения XSS через SVG-инъекции
   - Исправлен потенциально опасный regex в константах фронтенда

2. **CSRF (Межсайтовая подделка запросов)**
   - Реализована собственная CSRF-защита без использования устаревшего csurf
   - Генерация CSRF токенов через `randomBytes` с хранением в cookies
   - Проверка токенов для всех unsafe методов (POST, PUT, PATCH, DELETE)
   - Переведены GET-роуты `/auth/token` и `/auth/logout` на POST для предотвращения CSRF
...
3. **NoSQL-инъекция**
   ...
   - **Добавлена предварительная валидация ObjectId** в `orderController.ts`
   - ObjectId проверяется через `Types.ObjectId.isValid()` перед использованием в запросах
...
4. **Переполнение буфера**
   ...
   - Лимиты применены как в `src/app.ts`, так и в скомпилированном `dist/app.js`
...
5. **ReDoS (Regular Expression Denial of Service)**
   ...
   - **Исправлен потенциально опасный regex** в `frontend/src/utils/constants.ts`
...
7. **Path Traversal (Атака через обход директорий)**
   - **Добавлена явная проверка** `finalPath.startsWith(finalDir)` для предотвращения выхода за пределы директории
   - **Использование `path.resolve()`** для абсолютных путей
